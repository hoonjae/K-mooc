...
def main():
    ...
    while not (user_input is '0' or is_no(user_input)):
        ...
        while not(user_input is '0' or is_no(user_input)):
            ...
            if user_input == '0':
                ...
                SB_result = get_strikes_or_ball(user_input, random_number)
                print("Strikes : %d, Balls : %d" %(SB_result[0], SB_result[1]))

                if SB_result[0] == 3:
                    user_input = input("You win, One more(Y/N)?")
                    while not(is_no(user_input) or is_yes(user_input)):
